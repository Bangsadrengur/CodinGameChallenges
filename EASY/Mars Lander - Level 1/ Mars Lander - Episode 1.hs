...
increase power
  | power >= 4 = 4
  | otherwise = succ power
  
decrease power
  | power <= 0 = 0
  | otherwise = pred power

determinePower speed height power
  | speed  * speed - 5 * speed > height = increase power
...
loop = do
    ...
    let newPower = determinePower vspeed y power
    let output = "0 " ++ show newPower
